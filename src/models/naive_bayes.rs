...
impl<B: Backend<FloatElem = f32>> NaiveBayesClassifier<B> {
    ...
    pub fn log_probability_ratio(&self, x: &Tensor<B, 2>) -> Tensor<B, 1> {
        ...
        // reshape, not squeeze: a single row would squeeze [1, 1] down to nothing.
        let feature_sum = weighted_features.sum_dim(1).reshape([n_samples]);
        let prior_tensor =
...
pub fn generate_synthetic_language_data() -> (Vec<String>, Vec<String>) {
    // German: heavy on the orthography that actually distinguishes it —
    // sch, ei/ie, and the -ung / -heit / -keit / -schaft endings. Umlauts are
    // transliterated (oe, ae, ue) because the tokenizers here are a-z only.
    let german_words: Vec<String> = vec![
        "abend",
        "achtung",
        "allein",
        "anfang",
    ...
    // English: common vocabulary, with no word appearing in both lists.
    let english_words: Vec<String> = vec![
        "about",
        "account",
        "across",
        "action",
